    ...
    for (const user of users) {
      ...
      for (const st of settlements) {
        ...
        if (st.paidByUserId === user._id) {
          ...
          entry.amount -= st.amount; // reduces what user owes (or makes it negative -> they owe user)
          entry.since = Math.min(entry.since, st.date);

          if (Math.abs(entry.amount) < 0.01) ledger.delete(st.receivedByUserId);
        ...
        // Someone paid the user
          ...
          entry.amount += st.amount; // increases what user owes (or reduces what they owe user)
          // Wait.
          // If they owed user (amount < 0). They pay. Amount should increase (become less negative).
          // If user owed them (amount > 0). They pay (why?). Amount should increase (user owes more).
          // Correct.
          entry.since = Math.min(entry.since, st.date);

          if (Math.abs(entry.amount) < 0.01) ledger.delete(st.paidByUserId);
